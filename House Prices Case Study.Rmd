...
# fit2x <- stepAIC(lm(log_sale_price ~ . - sale_price, data = train),
#   scope = list(upper = ~. - sale_price, lower = ~ gr_liv_area),
#   direction = "forward", trace = FALSE
# )
# summary(fit2x)
# defaultSummary(data.frame(pred = predict(fit2x), obs = train$log_sale_price))
...
forward_pred <- data.frame(forward_pred)
names(forward_pred) <- c("id", "SalePrice")
